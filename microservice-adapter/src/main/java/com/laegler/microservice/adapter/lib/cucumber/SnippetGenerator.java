...
public class SnippetGenerator {
	...
	private static final Pattern[] ESCAPE_PATTERNS = new Pattern[] { Pattern.compile("\\$"), Pattern.compile("\\("),
	...
	public String getSnippet(Step step, FunctionNameGenerator functionNameGenerator) {
		return MessageFormat.format(snippet.template(), step.getKeyword().replaceAll(" ", ""),
				// I18n.codeKeywordFor(step.getKeyword()),
				snippet.escapePattern(patternFor(step.getText())), functionName(step.getText(), functionNameGenerator),
				snippet.arguments(argumentTypes(step)), REGEXP_HINT, snippet.tableHint());
	}
