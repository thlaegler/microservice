...
public class UsersRestFeatureSteps {
	...
	public void given_is_a_authorized_user_with_username_and_password(String username, String password) throws Throwable {
		token = feature.obtainAccessToken(username, password);
	}

	@Given("^is a User with username \"([^\"]*)\" and email \"([^\"]*)\"$")
	public void given_is_a_User_with_username_and_email(String username, String email) throws Throwable {
		user = User.builder().username(username).email(email).build();
	}

	@Given("^is a User with id (\\d+) and username \"([^\"]*)\" and email \"([^\"]*)\"$")
	public void given_is_a_User_with_id_and_username_and_email(int id, String username, String email) throws Throwable {
		user = User.builder().userId(Long.valueOf(id)).username(username).email(email).build();
	}

	@Given("^is a User with id (\\d+)$")
	public void given_is_a_User_with_id(int id) throws Throwable {
		user = User.builder().userId(Long.valueOf(id)).build();
	}

	@Given("^is a User with username \"([^\"]*)\"$")
	public void given_is_a_User_with_username(String username) throws Throwable {
		user = User.builder().username(username).build();
	}

	@When("^the authorized user adds the user$")
	public void when_the_authorized_user_adds_the_user() throws Throwable {
		resp = req.body(user).post("/");
	}

	@When("^the authorized user updates the user$")
	public void when_the_authorized_user_updates_the_user() throws Throwable {
		resp = req.body(user).put("/{id}", user.getUserId());
	}

	@When("^the authorized user deletes the user$")
	public void when_the_authorized_user_deletes_the_user() throws Throwable {
		resp = req.delete("/{id}", user.getUserId());
	}

	@When("^the authorized user requests the user by ID$")
	public void when_the_authorized_user_requests_the_user_by_ID() throws Throwable {
		resp = req.get("/{id}", user.getUserId());
	}

	@When("^the authorized user requests the user by username$")
	public void when_the_authorized_user_requests_the_user_by_username() throws Throwable {
		resp = req.get("/{username}", user.getUsername());
	}

	@When("^the authorized user requests all users$")
	public void when_the_authorized_user_requests_all_users() throws Throwable {
		resp = req.get();
	}

	@Then("^the response code should be (\\d+)$")
	public void then_the_response_body_should_contain_the_user(int statusCode) throws Throwable {
		assertEquals(resp.getStatusCode(), statusCode);
	}

	@Then("^the response body should contain the user$")
	public void then_the_response_body_should_contain_the_user() throws Throwable {
		assertEquals(resp.getBody().jsonPath().get("username"), user.getUsername());
	}

	@Then("^the response body should be empty$")
	public void then_the_response_body_should_be_empty() throws Throwable {
		assertNotNull(resp.getBody().jsonPath().get("username"), user.getUsername());
		assertEquals(resp.getBody().jsonPath().get("username"), user.getUsername());
	}

	@Then("^the response body should contain a list of users$")
	public void then_the_response_body_should_contain_a_list_of_users() throws Throwable {
